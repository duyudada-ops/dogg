-- Complete TailCircle production migrations

-- Remaining tables for events system
...
CREATE POLICY "Users can update their message images" ON storage.objects
  FOR UPDATE USING (bucket_id = 'message-images' AND auth.uid()::text = (storage.foldername(name))[1]);

CREATE POLICY "Users can delete their message images" ON storage.objects
  FOR DELETE USING (bucket_id = 'message-images' AND auth.uid()::text = (storage.foldername(name))[1]);
...
CREATE POLICY "Users can update their marketplace images" ON storage.objects
